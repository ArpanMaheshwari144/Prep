...
```
   ...
   NON-FUNCTIONAL (core):  FANOUT (1 event -> many channels+users) . ASYNC (producer block na ho)
                           . RELIABLE (retry + DLQ) . SCALABLE (peak ~58k/sec) . PRIORITY (OTP fast, marketing slow)
                           . PREFERENCES (user ka channel-choice honor).
...
```
   ...
   -> WRITE-HEAVY + SPIKY -> QUEUE (Kafka) absorb ; scale -> partitions x workers.
```
...
```
   Producers (Order/Payment/User) -> KAFKA topic "notifications" -> Notification Service
        ...
        -> per-channel QUEUES (push/email/sms) -> WORKERS -> Providers (FCM/SES/Twilio) -> USER -> Tracking DB
   box KYUN:
     Kafka   -> producer se decouple + spike absorb
...
```
   ...
   IDEMPOTENCY   -> ack kho gaya -> retry -> DUPLICATE (2 email).
                    fix: Redis key notification:id -> EXISTS? skip : send+mark. (at-least-once + idempotent worker.)
   RETRY + BACKOFF -> fail -> 1s,2s,4s,8s wait (dying service hammer na karo) + JITTER (thundering-herd rok).
   DLQ           -> max-retries paar -> Dead-Letter Queue -> manual review (poison msg: bad email/invalid phone).
```

## STEP 7 — BOTTLENECK / SCALE
```
   PRIORITY lanes:  OTP=HIGH (fast pool) . orders=MEDIUM . marketing=LOW  (alag worker-pools / PriorityQueue).
