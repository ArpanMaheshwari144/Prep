...
> ★ Ye theory hai; iske neeche wala **"SENTINEL LIVE"** hissa isi ko CHALA KE dekha hua hai (3 sentinel, asli failover).
...
## ═══ HANDS-ON — SENTINEL LIVE: KHUD-B-KHUD FAILOVER (18-Sep) ═══

> Upar wale demo me promote MAINE haath se kiya tha (`REPLICAOF NO ONE`) aur usi se SPLIT BRAIN hua.
> Ab wahi kaam 3 Sentinel se karaya — bina kuch chhue. Poora faisla logs me timestamp ke saath neeche hai.
> File: `07_PROJECTS/usercrud/docker-compose.yml` (sentinel-1 / sentinel-2 / sentinel-3)
...
```yaml
  sentinel-1:
    ...
    ports:
      - "26379:26379"                     # sentinel-2 -> 26380:26379 , sentinel-3 -> 26381:26379
    depends_on:
      - redis-master
    command: sh -c "IP=$$(getent hosts redis-master | cut -d' ' -f1); printf 'port 26379\nsentinel monitor mymaster %s 6379 2\nsentinel down-after-milliseconds mymaster 5000\nsentinel failover-timeout mymaster 10000\nsentinel parallel-syncs mymaster 1\n' $$IP > /tmp/sentinel.conf; redis-sentinel /tmp/sentinel.conf"
    restart: unless-stopped
```

Chalu hote waqt ek baar master ka IP nikaal ke config me likh deta hai. Container ke andar bani file:
...
# Generated by CONFIG REWRITE          <- ★ Sentinel ne file KHUD badal di
dir "/data"
```

★ `CONFIG REWRITE` wali line hi wajah hai ki **har Sentinel ko apni alag config file chahiye** — wo usme
naya master, doosri Sentinels ke pate, sab likhti rehti hai. Ek shared file rakhoge to teeno ek doosre pe likhengi.

Teeno chalu hone ke baad:

```
docker exec sentinel-1 redis-cli -p 26379 SENTINEL master mymaster
   ip                  : 172.19.0.2
   flags               : master          <- tandurust
   num-slaves          : 1
   num-other-sentinels : 2               <- teeno ek doosre ko dhoondh chuki
   quorum              : 2
```

### ★ GALTI 1 — `resolve-hostnames yes` ne TILT MODE me daal diya

Pehli koshish me maine `sentinel monitor mymaster redis-master 6379 2` (hostname) diya tha,
`sentinel resolve-hostnames yes` ke saath — soch ke ki IP badalne ki dikkat nahi aayegi. Ulta pad gaya:

```
+sdown master mymaster redis-master 6379        <- "shayad mara" tak pahunchi
Failed to resolve hostname 'redis-master'
+tilt #tilt mode entered
Failed to resolve hostname 'redis-master'
+tilt #tilt mode entered
... (5 minute tak wahi, failover kabhi nahi hua)
```

Container marte hi Docker ke DNS se naam gayab:

```
docker exec sentinel-1 nslookup redis-master
   ** server can't find redis-master: NXDOMAIN
```

**TILT MODE kya hai:** Sentinel ka suraksha-kawach. Jab use lage ki uske apne do check ke beech bahut
zyada waqt beet gaya (machine hang ho gayi ho jaise), to wo maan leti hai "mere naap pe bharosa nahi" aur
**koi faisla lena band** kar deti hai — sirf dekhti rehti hai. Isliye `+sdown` se aage `+odown` tak
pahunchi hi nahi. Logs me har resolve-failure ke turant baad tilt aaya — seedha matlab: naam hal karne me
laga waqt hi Sentinel ka loop late kara raha tha.

→ **SABAK:** `sentinel monitor` me **IP** do, hostname nahi. Deploy ke waqt IP bhar do (jaisa upar
`getent` wali line karti hai). Redis docs bhi yahi kehti hain.

### ★ GALTI 2 — YAML ne command ko DO TUKDE kar diya

Dusri koshish me sentinel container crash-loop me chala gaya:

```
sh: /tmp/sentinel.conf: Permission denied
# Sentinel needs config file on disk to save state. Exiting...
```

Do error, do alag baatein: pehla kehta hai shell us file ko **chalane** ki koshish kar raha tha,
dusra kehta hai `redis-sentinel` **bina config** chala. Matlab ek line do tukdon me tooti thi:

```
... > /tmp/sentinel.conf && redis-sentinel      <- config ke bina chal gaya
/tmp/sentinel.conf                              <- ise COMMAND samajh ke chalane ki koshish
```

**Wajah — YAML ka folded scalar (`>`):** usme agli lines **pehli line ke barabar** indent honi chahiye,
tabhi newline space me badalta hai. Editor ne auto-indent karke unhe **zyada andar** kar diya — aur YAML
ka niyam hai ki zyada-indent wali line ka newline **jaisa hai waisa rakha jaata hai**. To
`redis-sentinel` ke baad asli newline ghus gaya.

→ **SABAK:** compose me lamba `sh -c "..."` ho to `>` mat use karo — **ek hi physical line** me likho.
Aur `docker compose config` hamesha chala ke dekh lo, wo aisi cheezein pehle hi pakad leta hai.
(Isi command ne pehle `container_name` teeno me `sentinel-1` reh jaane wali galti bhi pakdi thi.)

### ★★ ASLI TEST — master ko goli, haath kuch nahi lagaya

```
docker exec redis-master redis-cli SET before:failover haan
docker kill redis-master

# 10 second baad --
docker exec redis-replica redis-cli INFO replication | findstr /B role
   role:master                                  <- KHUD BAN GAYA
docker exec redis-replica redis-cli GET before:failover
   haan                                         <- data bhi salamat
```

**Sentinel ke logs — poora faisla, 800 millisecond me:**

```
15:41:31.330  # +sdown master mymaster 172.19.0.2 6379          "mujhe nahi dikh raha"
15:41:31.391  # +odown master mymaster ... #quorum 3/2          teeno ne haan kaha (chahiye the 2)
15:41:31.391  # +try-failover master mymaster
15:41:31.398  # +vote-for-leader e0ba35f8... 1
15:41:31.410  * 313d5887... voted for e0ba35f8... 1             doosri Sentinel ka vote
15:41:31.410  * da13681e... voted for e0ba35f8... 1             teesri ka vote
15:41:31.457  # +elected-leader master mymaster
15:41:31.457  # +failover-state-select-slave
15:41:31.510  # +selected-slave slave 172.19.0.3:6379
15:41:31.510  * +failover-state-send-slaveof-noone slave 172.19.0.3   <- REPLICAOF NO ONE bheja
15:41:32.063  # +promoted-slave slave 172.19.0.3:6379
15:41:32.063  # +failover-state-reconf-slaves
15:41:32.131  # +switch-master mymaster 172.19.0.2 6379 172.19.0.3 6379
```

Ye line-by-line wahi hai jo maine pehle haath se kiya tha — par beech me **do kaam aur** hue jo haath se
ho hi nahi sakte the:

```
#quorum 3/2       -> ek Sentinel ke kehne pe kuch nahi hua. TEENO ne dekha, tab bada.
vote-for-leader   -> Sentinels ne chunav karaya ki failover KAUN chalayega
                     -> isi se pakka hua ki do Sentinel do ALAG replica promote na kar dein
```

★ `down-after` 5 second ka tha, par `+sdown` se `+switch-master` tak sirf **800 ms** lage.
**Faisla lene me der nahi hoti — der "pakka mar gaya" MAAN-NE me lagti hai.** (yahi `down-after` ka trade-off)

### ★★ SPLIT BRAIN HUA HI NAHI

```
docker start redis-master        # purana master wapas

docker exec redis-master redis-cli INFO replication
   role:slave                                   <- MASTER NAHI
   master_host:172.19.0.3
   master_link_status:up
```

Aur asli baat — Sentinel ne use **girte hi** replica ghoshit kar diya tha, lautne ka intezaar bhi nahi kiya:

```
15:41:32.132  * +slave slave 172.19.0.2:6379 @ mymaster 172.19.0.3   <- girte hi list me "replica"
15:44:12.412  # -sdown slave 172.19.0.2:6379 @ mymaster 172.19.0.3   <- 3 min baad lauta, seedha replica
```

Pichhli baar (haath wale demo me) yahi container `bangalore` leke `role:master` ban ke baitha tha.
Farak sirf ek kaam ka hai jo insaan bhool jaata hai (main bhoola tha):
**girne ke waqt hi yaad rakh lena ki ab maalik badal chuka hai.**

### DONO TAREEKE SAATH-SAATH — yahi is poore demo ka nichod

```
                          HAATH SE                    SENTINEL SE
------------------------------------------------------------------------------
master gira               write band                  write band (~1 sec)
"mara hai" kisne maana    maine, AKELE                teen me se teen (#quorum 3/2)
promote kisne kiya        maine                       chuni hui leader Sentinel
kitna waqt laga           jitna mujhe pata chala      800 ms
purana master lauta       role:master -> SPLIT BRAIN  role:slave, chup
data ka kya hua           "bangalore" MARA            kuch nahi khoya
```

→ Sentinel ne koi naya jaadu nahi kiya. Usne wahi teen kaam kiye jo main bhool gaya tha:
**bahumat se maano · promote karne wala ek hi ho · lautne wale ko turant replica banao.**
