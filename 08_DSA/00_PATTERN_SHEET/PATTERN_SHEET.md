...
```
 ...
 └──────────────────────────────────────────────────────────────
     ...
     COIN  p[i-1]*p[k]*p[j+1]  kaise (k = range ka SABSE-LAST phoota):
          p[i-1]   |<------- range [i .. j] ------->|   p[j+1]
          baahar-L         andar sab pehle gaye        baahar-R
                    \_____ k akela bacha (LAST) _____/
          k ke padosi = range ke BAAHAR wale -> coin = p[i-1] * p[k] * p[j+1]
