...
## ★★★ 28-JUL — READ FIRST (CURRENT STATE + AAGE KAISE JANA) — purane dated-updates neeche = historical
```
   WHERE WE ARE (28-Jul):
   - DSA: ~80-85% patterns DONE. Trees GATE-DONE (15). ★★ GRAPHS (28-Jul, 5 solved KHUD): Path-Exists(1971) · Islands(200) · Connected-Components(323) · Rotting-Oranges(994) · Max-Area-of-Island(695). LIBRARY me: BFS+DFS+visited+adj-list+outer-loop-count+multi-source-BFS+DFS-return-value. ★ Course-Schedule/topo-sort(Kahn's indegree) RECALLED (kabhi kiya, code-dekhte-turant-wapas = knowledge tha) -> AGLA = ise SHEET karna. Dijkstra samajhdaari se PARK (weighted = naya tier, base pehle). GRAPHS sheet-section = BROAD-IDEA + adj-list + DECISION-GUIDE(Q1 outer-loop? / Q2 DFS-BFS-code / Q3 adj-vs-grid + dirs-trick) + FAMILY-boxes(KYUN-SAATH) + 5 entries (all CONNECT-TO-TREES form). Arpan ne CC=Islands=Path-Exists, rotting=level-order connections KHUD dekhe. bache families = DP/Backtracking/Heap/Intervals.
   - PROJECT: mini_payment_ms + todoapp REVISE KHATAM (28-Jul). todoapp = poora JWT-auth-flow + Redis-caching(@Cacheable/@CacheEvict + RedisConfig JSON-serializer) + models(User/Todo, JPA/validation) + controllers(Todo/User REST) + DTOs(Register/Login/Token/UserResponse) + REPOS(JpaRepository + findByEmail/findByUserId derived-query) + DEPLOYMENT(Dockerfile[JRE] + docker-compose[app+mysql+redis, healthcheck/volumes] + k8s[deployment/service-NodePort/mysql/redis]). ★ 28-Jul FIX: UserResponse DTO banaya (password-leak: controller entity seedha return kar raha tha -> DTO se strip; static-factory from() = mapping ek jagah DRY). K8s THEORY = 02_SPRING/07_K8S (pod/deployment/service/ingress/configmap-secret, done). [todoapp files RICHLY code-commented = notes wahin].
   ...
   2. ★ PROJECT-REVISE: mini_payment + todoapp DONE (28-Jul) -> phir ROADMAP: 05_INFRA_DEEP -> 06_COMPARES -> 09_DATABASE -> 04_HLD/FOUNDATIONS (hand-note mode, Claude ACTUAL note/code dikhata, NO quiz; NON-obvious dikhao, obvious skip, Arpan khud curate).
   3. HLD mock roz (stock-trading -> baaki).
   4. ★ JAVA MULTITHREADING/CONCURRENCY extra-sharp (JP ka biggest differentiator).
   5. DSA-Graph: 5 done -> AGLA = course-schedule/topo-sort (recalled -> SHEET karna) -> cycle-detect / clone-graph / word-ladder -> phir DP/Backtracking/Heap/Intervals. (Dijkstra/DSU/MST = weighted tier, base-ke-baad.)
   ...
   ★★★ 28-JUL ROOT-CAUSE (Arpan-derived KHUD, HUGE -- poore 4-mahine DSA-crisis ka jawab, ek-line): DSA "hard" ki asli JAD = solved-but-NOT-sheeted+revised -> FADE. PROOF (uske apne 2 problems): TOPO-SORT/course-schedule (samjha YouTube, sheet-nahi-banayi -> 6-mahine-baad blur; PAR code-dekhte-hi-TURANT wapas = knowledge HAMESHA tha, sirf ANCHOR/sheet missing) vs MIN-WINDOW (sheet-banayi + revise -> PERMANENT, kabhi-bhi bina-help). same-tu/same-dimaag -> farak = SHEET+REVISE, ability NAHI. 1000-solved-problems "waste" nahi the, bas un-anchored -> fade; ab jo sheet+revise = permanent. => down-day pe ye DATA: "DSA nahi aati" GALAT tha, "sheet+revise nahi kiya" sahi -- ab habit hai. ★ COLD=IMPOSSIBLE (reinforce, cold=banned): un-seen trick "cold-invent" IMPOSSIBLE (max-product min-tracking proof -- koi 4-din-baith ke bhi nahi) -> "cold solve" framing = un-inventable pe FAIL-MARK = cruel+galat -> ego-crash -> folder-delete cycle. FIX = trick SEEKHO-once -> SHEET -> REVISE -> TERA.
