...
```
   ...
   DSA        ◐ METHOD = blank-produce + COPY-PEN (paper trace pehle) + spaced-redo. CODED = 7 patterns:
              ...
              ★ SPACED-REDO RULE (Arpan, 8-Jul): SABKO baar-baar redo karna hai — solo kiya ho ya peek, KOI farak nahi. "ek baar ho gaya" ≠ "pakka". har problem blank+copy-pen se dobara till automatic. (solo/peek tag = sirf note ki pehli baar kaisa gaya, filter NAHI — redo se koi bachta nahi.) abhi-pending redo: Subarray-K · + saare naye (matrix/stack/LL). (blank-redo SOLO done: Rotated ✓ · Longest-Substr ✓ · Koko ✓ 10-Jul -> pattern SET.)
